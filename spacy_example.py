...
def main(
    ...
    textcat = nlp.create_pipe(
        "trf_textcat",
        config={"architecture": "simple_cnn", "words_per_batch": max_wpb},
    )
...
def _prepare_partition(text_label_tuples, *, preprocess=False):
    ...
    cats = []
    train_cats = [str(digit) for digit in labels]
    unique_labels = list(set(train_cats))
    for label in labels:
        cat_dict = {i: False if int(i) != label else True for i in unique_labels}
